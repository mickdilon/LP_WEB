...
export class formularioTradicionalComponent implements OnInit {
  ...
  titulo_app_menu: string = "Tu profesión";
  ...
  constructor(
    ...
    this.getJsonService.getJsonEscuelasCRM();
    this.getJsonService.getCiclosSolvis();
    /**
     * 
     * 
     * 
     * Terminan los Json de Solvis
     * 
     * 
     * 
     * 
    ...
    //Traer json de catalogo de escuelas para CC de prospeccion
    ...
    /**
      * Login y validación al carreer cruising
    */
    jQuery("#vld_cc").on("click",function(){
        console.log("Esta intentado ingresar: " + jQuery("#codigoEscuelaCRM").val());
        console.log("Validando codigo");
        console.log(JSON.parse(localStorage.getItem("jsonEscuelasCRM")));
        var nodo_encontrado = that.readJsonService.buscar("crmit_claveescuelaempresacrm", jQuery("#codigoEscuelaCRM").val(), JSON.parse(localStorage.getItem("jsonEscuelasCRM")));
        console.log("EL NODO ENCONTRADO:")
        console.log(nodo_encontrado);
        console.log("lenght:" + nodo_encontrado.length);
        if(nodo_encontrado.length > 0 && nodo_encontrado.length < 2) {
          console.log("mostrar formulario");
          jQuery(".modal-body-form").removeClass("d-none");
          jQuery(".titulo-formulario").removeClass("d-none");
          jQuery(".a-c").addClass("d-none");
        }else{
          jQuery(".msj-esc").removeClass("d-none");
        }
    });

    /**
    *  Termina Login y validación al carreer cruising
    */
